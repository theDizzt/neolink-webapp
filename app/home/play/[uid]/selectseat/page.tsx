...
export default function TheaterSeatMap() {
  ...
  return (
    <div className="mx-auto max-w-4xl p-6">
      <h1 className="mb-8 text-center text-2xl font-bold">좌석 선택</h1>
      {/* 무대 나중에 이미지 삽입 해야됨 */}
      <div className="mb-8 rounded bg-blue-600 py-3 text-center text-white">
        무대
      </div>
            ...
            {seats
              ...
              .map((seat) => {
                const isInHoveredGroup =
                  hoveredGroup && seat.groupId === hoveredGroup.id; //호버 상태 확인
                ...
                return (
                  <button
                    key={seat.id}
                    onClick={() =>
                      seat.groupId && toggleGroupSelection(seat.groupId)
                    }
                    onMouseEnter={() => handleSeatHover(seat.id)}
                    onMouseLeave={() => setHoveredGroup(null)}
                    className={`flex h-20 w-20 justify-center rounded transition-colors ${
                      isSelected
                        ? 'bg-black'
                        : isInHoveredGroup
                          ? 'border-2 border-black/500 bg-gray-500'
